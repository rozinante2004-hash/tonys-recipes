...
**Current version:** `v36.19` — app. **Worker: v39**, deployed separately and versioned separately
...
| `version.json` | `{"version": "v36.19"}` — polled to detect new deployments. Must never be cached, and must be bumped in the same commit as `index.html`. |
