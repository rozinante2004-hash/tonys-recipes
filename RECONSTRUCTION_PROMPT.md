...
**Current version:** `v36.68` — app. **Worker: v41**, deployed separately and versioned separately
...
| `version.json` | `{"version": "v36.68"}` — polled to detect new deployments. Must never be cached, and must be bumped in the same commit as `index.html`. |
