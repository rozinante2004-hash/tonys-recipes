...
**Current version:** `v36.5` — app. **Worker: v38**, deployed separately and versioned separately
...
| `version.json` | `{"version": "v36.5"}` — polled to detect new deployments. Must never be cached, and must be bumped in the same commit as `index.html`. |
