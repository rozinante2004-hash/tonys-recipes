...
**Current version:** `v36.28` — app. **Worker: v40**, deployed separately and versioned separately
...
| `version.json` | `{"version": "v36.28"}` — polled to detect new deployments. Must never be cached, and must be bumped in the same commit as `index.html`. |
