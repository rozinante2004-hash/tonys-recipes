...
**Current version:** `v36.59` — app. **Worker: v41**, deployed separately and versioned separately
...
| `version.json` | `{"version": "v36.59"}` — polled to detect new deployments. Must never be cached, and must be bumped in the same commit as `index.html`. |
...
deletes itself rather than letting the error reach the caller. **Log an outcome where the outcome
happens** — v35.4 logged a successful write inside `saveRecipeDoc` and the refusal one level up,
which every unit test passed and an end‑to‑end browser run caught immediately; both halves live in
`saveRecipeDoc` now (v35.5), and the caller keeps only `markRecipeConflicted`, which is UI state.

**The nudge (v35.6):** the log only helps if someone looks, so the app mentions it — rather than
uploading itself anywhere, which was the alternative and is worse (nothing it could send reaches a
person who is not already looking, and it would park recipe names and family identities on a third
party). `maybeNudgeAboutLog()` shows a dismissible card with **Copy report** / **Look** / ✕ about
4s after start. **Off by default and per device** (`tonys_log_nudge`), so the desk can have it and
the phone need not. It fires at most **once per session** and **never twice for the same problems**
(`logNudgeSignature` hashes the `bad` findings' titles, which carry counts — a fourth refusal is
news, the same three are not). Every path returns a **named reason**, so "stayed quiet correctly"
is distinguishable from "silently broken".
...
dropdown (Family Access, Gmail Setup, Self Test, 🪵 Logging & debugging, Payments, Deployments, plus the photo tools —
...
- **Word** (`.docx`): **built by hand** as an OOXML zip — `makeDocxBlob` + a tiny `buildZip`
  ...
  - **The picture is `<wp:inline>`, never `<wp:anchor>`.** Inline gives it a line box of its own, so
    ...
    ingredients.
  - **The heading is the recipe name only — no `r.emoji`.** The emoji is the app's
    on-screen stand-in for a missing photo; a printed document gets the photo itself, and
    the glyph rendered as a tofu box wherever the reader's font lacked it.
...
- **Sub-titles (v36.4)** — heading lines inside a recipe's ingredients AND its method,
  with per-line indentation. Shapes: an ingredient is `{a,n,ind?}` or `{sub}`; a step is a
  plain string, `{t,ind}` when indented, or `{sub}`. `normalizeIngredientLines` /
  `normalizeStepLines` own the coercion and are used by BOTH `normalizeRecipe` and
  `normalizePart`, so a list renders the same wherever it lives. They also fold v36.0's
  per-ingredient `g` into heading lines — **there is one grouping mechanism, not two**.
  **`allIngredients`/`allSteps` mean the COOKABLE CONTENT and always have**; the ~20
  consumers (search, pantry, one-away, nutrition, Bring!, scaling, Word, print, share)
  depend on that. `ingredientLines`/`stepLines` include the headings and are for
  renderers only. `ingredientLinesHtml`/`stepLinesHtml` draw both the ordinary view and a
  collection's sections; step numbers count real steps only, and a heading gets no
  `data-line`, so it cannot be ticked. `stepsToText`/`textToSteps` are the textarea form
  (`# ` heading, leading spaces indent) that the method toolbar writes for you.
  `reapplyLineStructure` restores the shape after translation and **drops it when the
  line counts disagree** rather than guessing.
