...
| ~~5f.7~~ | ✅ **DONE (v29.5)** — ~~uploading a chat needed two steps~~ | Adding a WhatsApp export meant uploading the file *and* hand-editing `index.json`, which is what made it a laptop job. The app now lists the folder over the GitHub contents API, so dropping a file in is enough; `index.json` is optional and only supplies group labels, and is still used when the listing is unavailable (private repo, non-Pages folder). `whatsapp/UPLOAD-FROM-IPHONE.md` has the Share-sheet Shortcut. | 🟢 |
| ~~5f.8~~ | ✅ **DONE (v29.6)** — ~~a blocked device cannot get a chat in or out~~ | Tony's iPhone is employer-managed and blocks `github.com` and `api.github.com` outright, so the folder, the Shortcut and `upload.html` all fail on it — the Shortcut was 25 steps in before this surfaced. Chats now also travel through **Firestore**, the same place recipes already live and the same sign-in: `chat_<slug>` metadata plus `chatpart_<slug>_<n>` text chunked by **bytes** (Hebrew is 2 bytes/char, emoji 4, so a character-sized chunk is double what you budgeted). Listing is cheap because the head range query cannot see the parts; text is fetched on demand and cached in IndexedDB. Import on the phone → appears everywhere. A ☁️ Sync button pushes an existing folder chat up once. Removing a synced chat asks first, because it removes it for everybody. No rules change — `match /shared/{document=**}` already covered it. | 🟢 |
| ~~5f.9~~ | ✅ **DONE (v30.4)** — ~~free-hand text could only make a NEW recipe~~ | Tony has recipes saved as clips (a video link, no ingredients, "Enjoy!" for a method) and has since found the text for some. The free-hand parser only ever created a new recipe, so using it meant a duplicate to merge by hand. The edit form now has **✍️ Paste free-hand text to fill this in**: it parses, previews, and offers *Fill in what's missing* or *Replace ingredients & method*. It writes to the FORM, never to storage — nothing is saved until Update Recipe, so the v29.4 open-form conflict guard still applies. The name and photo are kept either way; "Enjoy!" counts as empty, since that is the exact case it exists for. | 🟢 |
